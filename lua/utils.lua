...
function M.switch_to_previous_buffer()
  local ok, _ = pcall(function() vim.cmd "buffer #" end)
  if not ok then vim.notify("No other buffer to switch to!", 3, { title = "Warning" }) end
end
...
function M.get_buffer_count()
  ...
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.fn.bufname(buf) ~= "" then count = count + 1 end
  end
  return count
end
...
function M.parse_hex(int_color) return string.format("#%x", int_color) end
...
function M.telescope_diff_file(recent)
  ...
  if recent then picker = require("telescope.builtin").oldfiles end
